...
from dgtd.sample_dgtd import build_mode_a_time_grid, export_dp_schedule_stub, rollout_mode_a, summarize_mode_a
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=["mode_a", "mode_b_stub"], default="mode_a", help="DGTD sampling mode")
...
def main() -> None:
    ...
    samples, time_grid = rollout_mode_a(
        model=model,
        x_init=noise,
        warp=timewarp,
    ...
    print(f"time_grid: {summarize_mode_a(build_mode_a_time_grid(warp=timewarp, step_count=args.steps, device=device, dtype=torch.float32))['time_grid']}")
