...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run official CTM CIFAR-10 samples and EDM FID eval.")
    parser.add_argument(
        "--checkpoint",
    ...
    parser.add_argument("--fid-ref", default="https://nvlabs-fi-cdn.nvidia.com/edm/fid-refs/cifar10-32x32.npz")
    ...
    parser.add_argument("--class-cond", action=argparse.BooleanOptionalAction, default=True)
            ...
            writer.writerow(
                {
                    ...
                    "eval_script": "scripts/baselines/run_ctm_cifar10_eval.py",
                    "notes": row["notes"],
                }
            )
...
def main() -> None:
    ...
    for step_count in args.steps:
        ...
        if completed_row is not None and not args.skip_fid:
            ...
            print(f"reuse ctm cifar10 step_count={step_count} fid={completed_row['fid']}", flush=True)
            continue
        ...
        row = {
            ...
            "notes": "; ".join(
                [
                    ...
                    "recall pending CIFAR-10 reference sample batch",
                ]
            ),
        }
        ...
        rows.append(row)
        _write_outputs(rows=rows, eval_root=eval_root, csv_out=csv_out)
        print(f"ctm cifar10 step_count={step_count} fid={fid} elapsed_sec={row['elapsed_sec']:.2f}", flush=True)
