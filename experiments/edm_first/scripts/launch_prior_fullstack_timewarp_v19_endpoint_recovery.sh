...
if [[ $# -lt 1 ]]; then
  echo "Usage: bash experiments/edm_first/scripts/launch_prior_fullstack_timewarp_v19_endpoint_recovery.sh <source_checkpoint> [run_tag] [tmux_session]" >&2
  exit 2
fi

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
source_checkpoint="$1"
config="${DG_TWFD_FULLSTACK_CONFIG:-experiments/edm_first/configs/cifar10_edm_map_prior_fullstack_timewarp_v19_endpoint_recovery.yaml}"
baseline_summary="${DG_TWFD_EVAL_BASELINE_SUMMARY:-eval/edm_first_cifar10_endpoint_ema_v18_from_v17_last_step4684/reports/summary.json}"
...
run_tag="${2:-edm_first_cifar10_prior_fullstack_timewarp_v19_endpoint_recovery_from_v18_step${source_step}}"
tmux_session="${3:-v19_endpoint_recovery}"

if tmux has-session -t "$tmux_session" 2>/dev/null; then
  echo "fullstack timewarp v19 endpoint recovery train already running: ${tmux_session}"
else
  tmux new-session -d -s "$tmux_session" \
    "cd '$ROOT_DIR' && bash experiments/edm_first/scripts/launch_train.sh '$config' '$run_tag' '$source_checkpoint'"
  echo "fullstack timewarp v19 endpoint recovery train started: ${tmux_session}"
fi

if tmux has-session -t "${tmux_session}_eval_watch" 2>/dev/null; then
  echo "fullstack timewarp v19 endpoint recovery eval watcher already running: ${tmux_session}_eval_watch"
else
  ...
  echo "fullstack timewarp v19 endpoint recovery eval watcher started: ${tmux_session}_eval_watch"
fi

if tmux has-session -t "${tmux_session}_backup" 2>/dev/null; then
  echo "fullstack timewarp v19 endpoint recovery project backup watcher already running: ${tmux_session}_backup"
else
  ...
  echo "fullstack timewarp v19 endpoint recovery project backup watcher started: ${tmux_session}_backup"
fi

if [[ "${DG_TWFD_START_HOURLY_SUPERVISOR:-1}" == "1" ]]; then
  if tmux has-session -t "${tmux_session}_2h" 2>/dev/null; then
    echo "fullstack timewarp v19 endpoint recovery 2h supervisor already running: ${tmux_session}_2h"
  else
    ...
    echo "fullstack timewarp v19 endpoint recovery 2h supervisor started: ${tmux_session}_2h"
  fi
fi
