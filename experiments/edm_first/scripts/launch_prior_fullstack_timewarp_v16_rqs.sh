...
if [[ $# -lt 1 ]]; then
  echo "Usage: bash experiments/edm_first/scripts/launch_prior_fullstack_timewarp_v16_rqs.sh <source_checkpoint> [run_tag] [tmux_session]" >&2
  exit 2
fi

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
source_checkpoint="$1"
config="${DG_TWFD_FULLSTACK_CONFIG:-experiments/edm_first/configs/cifar10_edm_map_prior_fullstack_timewarp_v16_rqs.yaml}"
...
run_tag="${2:-edm_first_cifar10_prior_fullstack_timewarp_v16_rqs_from_step${source_step}}"
tmux_session="${3:-v16_rqs_tw}"

if tmux has-session -t "$tmux_session" 2>/dev/null; then
  echo "fullstack timewarp v16 RQS train already running: ${tmux_session}"
else
  tmux new-session -d -s "$tmux_session" \
    "cd '$ROOT_DIR' && bash experiments/edm_first/scripts/launch_train.sh '$config' '$run_tag' '$source_checkpoint'"
  echo "fullstack timewarp v16 RQS train started: ${tmux_session}"
fi

if tmux has-session -t "${tmux_session}_eval_watch" 2>/dev/null; then
  echo "fullstack timewarp v16 RQS eval watcher already running: ${tmux_session}_eval_watch"
else
  ...
  echo "fullstack timewarp v16 RQS eval watcher started: ${tmux_session}_eval_watch"
fi

if tmux has-session -t "${tmux_session}_backup" 2>/dev/null; then
  echo "fullstack timewarp v16 RQS project backup watcher already running: ${tmux_session}_backup"
else
  ...
  echo "fullstack timewarp v16 RQS project backup watcher started: ${tmux_session}_backup"
fi

if [[ "${DG_TWFD_START_HOURLY_SUPERVISOR:-1}" == "1" ]]; then
  if tmux has-session -t "${tmux_session}_2h" 2>/dev/null; then
    echo "fullstack timewarp v16 RQS 2h supervisor already running: ${tmux_session}_2h"
  else
    ...
    echo "fullstack timewarp v16 RQS 2h supervisor started: ${tmux_session}_2h"
  fi
fi
