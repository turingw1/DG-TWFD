...
if [[ $# -lt 1 ]]; then
  echo "Usage: bash experiments/edm_first/scripts/launch_prior_fullstack_timewarp_v20_endpoint_balanced.sh <source_checkpoint> [run_tag] [tmux_session]" >&2
  exit 2
fi

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
source_checkpoint="$1"
config="${DG_TWFD_FULLSTACK_CONFIG:-experiments/edm_first/configs/cifar10_edm_map_prior_fullstack_timewarp_v20_endpoint_balanced.yaml}"
baseline_summary="${DG_TWFD_EVAL_BASELINE_SUMMARY:-eval/edm_first_cifar10_prior_fullstack_timewarp_v19_endpoint_recovery_bs48_from_v18_step4684_step7106_budget/reports/summary.json}"
...
run_tag="${2:-edm_first_cifar10_prior_fullstack_timewarp_v20_endpoint_balanced_from_v19_step${source_step}}"
tmux_session="${3:-v20_endpoint_balanced}"

if tmux has-session -t "$tmux_session" 2>/dev/null; then
  echo "fullstack timewarp v20 endpoint balanced train already running: ${tmux_session}"
else
  tmux new-session -d -s "$tmux_session" \
    "cd '$ROOT_DIR' && bash experiments/edm_first/scripts/launch_train.sh '$config' '$run_tag' '$source_checkpoint'"
  echo "fullstack timewarp v20 endpoint balanced train started: ${tmux_session}"
fi

if tmux has-session -t "${tmux_session}_eval_watch" 2>/dev/null; then
  echo "fullstack timewarp v20 endpoint balanced eval watcher already running: ${tmux_session}_eval_watch"
else
  ...
  echo "fullstack timewarp v20 endpoint balanced eval watcher started: ${tmux_session}_eval_watch"
fi

if tmux has-session -t "${tmux_session}_backup" 2>/dev/null; then
  echo "fullstack timewarp v20 endpoint balanced project backup watcher already running: ${tmux_session}_backup"
else
  ...
  echo "fullstack timewarp v20 endpoint balanced project backup watcher started: ${tmux_session}_backup"
fi

if [[ "${DG_TWFD_START_HOURLY_SUPERVISOR:-1}" == "1" ]]; then
  if tmux has-session -t "${tmux_session}_2h" 2>/dev/null; then
    echo "fullstack timewarp v20 endpoint balanced 2h supervisor already running: ${tmux_session}_2h"
  else
    ...
    echo "fullstack timewarp v20 endpoint balanced 2h supervisor started: ${tmux_session}_2h"
  fi
fi
